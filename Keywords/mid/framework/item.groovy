...
public class item {
	...
	def editItem(boolean isView = false, boolean isRandom = false, int incrementID = 0) {
		...
		if (isRandom) {
			...
			item = generalAction.createTestObject("//div[@id='table-with-scroll']/div[contains(@class, 'row-table-custom')]["+randomNumber+"]//*[contains(@class, 'feather-edit')]")
		} else {
			item = generalAction.createTestObject("//div[@id='table-with-scroll']/div[contains(@class, 'row-table-custom')]["+incrementID+"]//*[contains(@class, 'feather-edit')]")
		}
