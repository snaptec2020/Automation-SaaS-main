...
boolean captchaErrorFound=true
int trials=1
while(captchaErrorFound & trials<10) {
	WebUI.delay(1)
	trials++
	if(trials>=10) {
		KeywordUtil.markFailedAndStop("Unable to login")
	}
	...
	WebUI.delay(5)
	TestObject errTO= new TestObject()
	errTO.addProperty("xpath", ConditionType.EQUALS, '//div[contains(text(),"reCAPTCHA verification failed.")]')
	if(WebUI.waitForElementPresent(errTO, 10)) {
		captchaErrorFound=true
	}else {
		captchaErrorFound=false
	}
}
...
String getOTPFromBE(){
	...
	Set<String> allHandles = DriverFactory.getWebDriver().getWindowHandles()
	if(allHandles.size().equals(2)) {
